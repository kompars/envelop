...
public enum class EmailAddressParseError {
     ...
     * character in the local-part, an `'@'` symbol, and at least one character
     * in the domain.
     */
    ADDRESS_TOO_SHORT,

    /**
     * An email address cannot begin with the `'@'` symbol, unless it has valid source
     ...
     * characters `'.', '-'` and whitespace. This failure reason indicates that a character
     ...
     * `[IPV4_addr]` or `[IPV6_addr]`. This failure reason indicates that the address
    ...
    /**
     * The local-part of an email address cannot end with the `'.'` character.
     */
    ...
    /**
     * An email address must contain the at `'@'` symbol exactly once.
     */
    MISSING_AT_SYMBOL,
    ...
    /**
     * An email address can only have a single unquoted `'@'` symbol. Multiple
     * `'@'` symbols can only appear within quoted parts, or escaped with a preceding
     * `'\'` character.
     */
    MULTIPLE_AT_SYMBOLS,

    /**
     * An email address cannot have two consecutive dot `'.'` characters outside of quotes.
     */
    MULTIPLE_DOT_SEPARATORS,

    /**
     * The TLD of an email address cannot be all numeric (ex: `test@hello.123`).
     */
    NUMERIC_TLD,

    /**
     * An email address cannot start with the `'.'` character.
     */
    STARTS_WITH_DOT,
    ...
    /**
     * An email address cannot contain the `'<'` character outside of quotes,
     * unless the address has an identifier.
     */
    UNQUOTED_ANGLED_BRACKET,

    /**
     * A backslash `'\'` within the local-part of an email address must be used to escape
